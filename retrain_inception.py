...
def create_image_lists(image_dir, list_dir, zoom_level):
  ...
  Args:
    ...
    list_dir: String path to a folder containing image address txt of different zoom levels
    zoom_level: String value of targeted zoom level e.g. 40X
  ...
  if not gfile.Exists(image_dir):
      ...
      return None
  if not gfile.Exists(list_dir):
      print("Split directory '" + list_dir + "' not found.")
      return None
  ...
  for set_ in ('train', 'val', 'test'):
    with open(os.path.join(list_dir, zoom_level + '_' + set_ + '.txt')) as f:
      for line in f:
...
def create_model_graph():
  ...
  with tf.Graph().as_default() as graph:
    model_path = os.path.join(FLAGS.model_dir, MODEL_FILENAME)
    with gfile.FastGFile(model_path, 'rb') as f:
      graph_def = tf.GraphDef()
      graph_def.ParseFromString(f.read())
      bottleneck_tensor, resized_input_tensor, jpeg_data_tensor = (
      tf.import_graph_def(
          graph_def,
          name='',
  ...
  return graph, bottleneck_tensor, resized_input_tensor, jpeg_data_tensor
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir,
  ...
  label_lists = image_lists[label_name]
  sub_dir = label_lists['dir'] #''
  # print("BOTTLENECK_DIR: ",bottleneck_dir,"\nSUB DIR: ",sub_dir)
  sub_dir_path = os.path.join(bottleneck_dir, sub_dir)
  ensure_dir_exists(sub_dir_path)
  bottleneck_path = get_bottleneck_path(image_lists, label_name, index,
                                        bottleneck_dir, category)
  # print("BOTTLENECKPATH: ",bottleneck_path)
...
def main(_):
  ...
  image_lists = create_image_lists(FLAGS.image_dir, FLAGS.list_dir, FLAGS.zoom_level)
  class_count = len(image_lists.keys()) #2
  if class_count == 0:
    ...
    if do_distort_images:
      ...
      (distorted_jpeg_data_tensor,
       distorted_image_tensor) = add_input_distortions(
           FLAGS.flip_left_right, FLAGS.random_crop, FLAGS.random_scale,
           FLAGS.random_brightness, INPUT_WIDTH,
